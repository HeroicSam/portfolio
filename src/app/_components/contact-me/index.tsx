      ...
      <motion.div className='page' {...anim(perspective)}>
        <motion.div {...anim(opacity)}>
          <form onSubmit={(handleSubmit(onSubmit))} className="flex flex-col gap-y-2 max-w-[500px]">
            ...
            <p className={`font-light mb-4`}>Email me at yuenc2@tcnj.edu or drop a message below.</p>
            ...
            <div className="w-full flex justify-end">
              <button type="submit" className="success text-sm py-1.5 px-4 border border-slate-200 dark:border-slate-700 dark:bg-slate-800 rounded-md">
                {isSuccess ? 'Message sent! 🎉' : isLoading ? <BeatLoader loading={isLoading} color="#06b6d4" size={4} /> : 'Send'}
              </button>
            </div>
          </form>
        </motion.div>
